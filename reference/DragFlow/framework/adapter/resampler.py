...
def reclaim_memory():
    if torch.cuda.is_available():
        ...
        for gpu_id in range(num_gpus):
            try:
                ...
                logging.error(f"GPU{gpu_id}MemoryReleaseError: {str(e)}")
        gc.collect()
    if torch.backends.mps.is_available():
        try:
            ...
            logging.error(f"MPSMemoryReleaseError: {str(e)}")
...
class CrossLayerCrossScaleProjector(nn.Module):
    ...
    def forward(self, low_res_shallow, low_res_deep, high_res_deep, timesteps, cross_attention_kwargs=None, need_temb=True, device_0="cuda:0", device_1="cuda:1"):
        '''
            low_res_shallow [bs, 729*l, c]
            low_res_deep    [bs, 729, c]
            high_res_deep   [bs, 729*4, c]
        '''
        low_res_deep = low_res_deep.to(device_1)
        low_res_shallow = low_res_shallow.to(device_1)
        high_res_deep = high_res_deep.to(device_1)
        timesteps = timesteps.to(device_1)
