...
class FluxIPAttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ip_hidden_states = self._get_ip_hidden_states(
            attn, 
            query if encoder_hidden_states is not None else query[:, emb_dict['length_encoder_hidden_states']:].to(device_1),
    ...
    def _get_ip_hidden_states(
        ...
        ip_hidden_states = ip_hidden_states.to(device_1)
        img_query = img_query.to(device_1)
        ...
        ip_query = ip_query.to(device_0)
        ip_key = ip_key.to(device_0)
        ip_value = ip_value.to(device_0)

        ip_value = attn.head_to_batch_dim(ip_value)
        ip_hidden_states = self._scaled_dot_product_attention(
            ip_query.to(ip_value.dtype),
            ip_key.to(ip_value.dtype),
            ip_value, None, attn.heads
        )
